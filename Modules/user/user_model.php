...
class User
{
    ...
    public function emon_session_start()
    {
        ...
        if (isset($_SESSION['cookielogin'])) {
            $session['cookielogin'] = $_SESSION['cookielogin'];
        } else {
            $session['cookielogin'] = 0;
        }
    ...
    public function register($username, $password, $email)
    {
        ...
        if ($userid == 1) {
            $this->mysqli->query("UPDATE users SET admin = 1 WHERE id = '1'");
        }
    ...
    public function login($username, $password, $remembermecheck)
    {
        ...
        if (!$username || !$password) {
            return array('success' => false, 'message' => _("Username or password empty"));
        }
    ...
    public function get_apikeys_from_login($username, $password)
    {
        ...
        if ($username_out != $username) {
            return array('success' => false, 'message' => _("Username must only contain a-z 0-9 dash and underscore"));
        }
        ...
        if ($hash != $userData->password) {
            ...
            return array(
                'success' => true,
                'apikey_write' => $userData->apikey_write,
                'apikey_read' => $userData->apikey_read
            );
        }
    }
            ...
            if ($userid > 0) {
                ...
                if ($enable_password_reset == true) {
                    ...
                    $message = Swift_Message::newInstance()
                        ...
                        ->setTo(array($email))
                        ->setBody("<p>A password reset was requested for your emoncms account.</p><p>Your can now login with password: $newpass </p>",
                            'text/html');
    ...
    public function get_timezones()
    {
        ...
        if ($timezones === null) {
            ...
            foreach (DateTimeZone::listIdentifiers() as $timezone) {
                ...
                $timezones[] = array(
                    'id' => $timezone,
                    'gmt_offset_secs' => $offset,
                    'gmt_offset_text' => $gmt_offset_text
                );
            }
        }
